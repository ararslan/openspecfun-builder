...
if [ -f "$private_libdir/libopenspecfun.$SHLIB_EXT" ]; then
    ...
    LIBGFORTRAN_PATH=$(find_shlib "$(private_libname openspecfun)" libgfortran)
    LIBGCC_PATH=$(find_shlib "$(private_libname openspecfun)" libgcc_s)
    LIBQUADMATH_PATH=$(find_shlib "$(private_libname openspecfun)" libquadmath)

    # Take the directories, add them onto LIBGFORTRAN_DIRS, which we use to
    # search for these libraries in the future.
    LIBGFORTRAN_DIRS="$(dirname $LIBGFORTRAN_PATH)"
    LIBGFORTRAN_DIRS="$(dirname $LIBGCC_PATH)"
    LIBGFORTRAN_DIRS="$(dirname $LIBQUADMATH_PATH)"

    # Save the SONAMES
    LIBGFORTRAN_SONAMES="$(basename "$LIBGFORTRAN_PATH")"
    LIBGCC_SONAMES="$(basename "$LIBGCC_PATH")"
...
for lib in libopenspecfun $SONAMES; do
